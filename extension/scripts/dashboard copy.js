...
function initDashboard() {
  ...
  // History Refresh button - use setTimeout to ensure DOM is ready
  setTimeout(() => {
    const historyRefreshBtn = document.getElementById('history-refresh-btn');
    if (historyRefreshBtn) {
      console.log('[Dashboard] History refresh button found, attaching listener');
      historyRefreshBtn.addEventListener('click', async (e) => {
        ...
        try {
          await loadHistory();
          console.log('[Dashboard] History refresh completed');
        } catch (error) {
          console.error('[Dashboard] History refresh error:', error);
          displayAlert('Failed to refresh history: ' + error.message, 'error');
        }
      });
    } else {
      console.warn('[Dashboard] History refresh button not found');
    }
  }, 100);
...
async function loadHistory() {
  try {
    ...
    historyRecords.forEach(record => {
      ...
      const extractedValues = record.extracted_values || {};
      const valuesPreview = Object.entries(extractedValues)
        .slice(0, 2)
        .map(([k, v]) => `${k}: ${v}`)
        .join(', ');
